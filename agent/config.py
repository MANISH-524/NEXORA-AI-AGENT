...
# Agent loop
# ---------------------------------------------------------------------------
CYCLE_SECONDS = _int("NEXORA_CYCLE_SECONDS", 60)
API_WS_URL = os.getenv("NEXORA_API_WS", "ws://localhost:8000/ws")
ENV_NAME = os.getenv("NEXORA_ENV", "development")
LOG_LEVEL = os.getenv("NEXORA_LOG_LEVEL", "info")
WORLD_TICK_SECONDS = _int("NEXORA_WORLD_TICK_SECONDS", 300)  # how often simulated asset state drifts

# ---------------------------------------------------------------------------
# LLM provider — pluggable. Any OpenAI-compatible endpoint works out of the box.
# ---------------------------------------------------------------------------
# Generic override surface. If LLM_PROVIDER is set, it wins. Otherwise NEXORA
...
# provider errors out mid-cycle, NEXORA retries once on this one before
...
USE_LOCAL_FALLBACK = _bool("NEXORA_USE_LOCAL", "false")
...
USE_SLM = _bool("NEXORA_USE_SLM", "false")
SLM_MAX_ASSETS = _int("NEXORA_SLM_MAX_ASSETS", 24)  # CPU budget per cycle
# Safety: when the SLM's action disagrees with NEXORA's deterministic policy,
...
SLM_STRICT = _bool("NEXORA_SLM_STRICT", "true")

LLM_TIMEOUT_SECONDS = _int("NEXORA_LLM_TIMEOUT", 30)
LLM_MAX_RETRIES = _int("NEXORA_LLM_MAX_RETRIES", 1)
...
CORS_ORIGINS = [o.strip() for o in os.getenv("NEXORA_CORS_ORIGINS", "*").split(",") if o.strip()]
