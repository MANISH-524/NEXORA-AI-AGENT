# NEXORA Local SLM — Fine-Tuned Reasoning Brain

NEXORA can run its recovery-risk reasoning on a **locally fine-tuned Small
...
1. **LoRA fine-tune** (`peft` + `transformers`) — a model specialised on
   NEXORA's own decision policy. Runs through `agent/reasoning/slm_local.py`.
2. **Ollama** — serve a ready-made or exported model locally; NEXORA already
...
## 1. Fine-tune the SLM on NEXORA data

The training data is generated from NEXORA's own deterministic policy
...
:: 2. LoRA fine-tune  ->  models/nexora-slm-lora/
...
```ini
NEXORA_USE_SLM=true
NEXORA_SLM_MAX_ASSETS=24      # CPU budget: assets SLM-reasoned per cycle
```
...
Point NEXORA at it as the **primary** provider in `.env`:
...
To unify both paths — run your NEXORA-specialised LoRA model inside Ollama —
...
:: then:
ollama create nexora-slm -f models\Modelfile.nexora
ollama run nexora-slm
```

See `models/Modelfile.nexora` for the template.
...
| File | Purpose |
|------|---------|
| `scripts/slm_dataset.py` | Generate SFT data from NEXORA's policy |
| `scripts/slm_train.py` | LoRA fine-tune (CPU) → `models/nexora-slm-lora/` |
| `scripts/slm_infer.py` | Evaluate the adapter vs ground truth |
| `agent/reasoning/slm_local.py` | Live local-SLM reasoning provider |
| `models/Modelfile.nexora` | Ollama export template |
