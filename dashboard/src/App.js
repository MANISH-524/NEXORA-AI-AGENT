...
import { API_BASE, api } from './lib';
import {
  LayoutDashboard, Radio, TrendingUp, Brain, Lightbulb,
  FlaskConical, Server, Map, RotateCcw, ScrollText,
} from 'lucide-react';
import './App.css';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      let sock;
      try {
        sock = new WebSocket(`${API_BASE.replace('http', 'ws')}/ws`);
      ...
      sock.onclose = () => { setWs(false); if (!stopped) setTimeout(connect, 3000); };
