"""
NEXORA — Local SLM Reasoner (fine-tuned)
========================================
Runs NEXORA's recovery-risk reasoning on a *locally fine-tuned* Small Language
Model (the LoRA adapter produced by scripts/slm_train.py), with zero cloud
dependency. This is the runtime for the "fine-tune an SLM on NEXORA data" path.
...
_ADAPTER_DIR = Path(__file__).resolve().parent.parent.parent / "models" / "nexora-slm-lora"
...
def _load():
    ...
    try:
        ...
        base_file = _ADAPTER_DIR / "nexora_base.txt"
        if not base_file.exists():
            raise RuntimeError("adapter missing nexora_base.txt — retrain with scripts/slm_train.py")
