"""
NEXORA — SLM LoRA Fine-Tuner
============================
Fine-tunes a small instruct model with LoRA so it reproduces NEXORA's
...
Pipeline:
    ...
    2. scripts/slm_train.py     -> models/nexora-slm-lora/  (LoRA adapter)
...
DATA = ROOT / "data" / "slm"
OUT = ROOT / "models" / "nexora-slm-lora"
...
def main(args):
    ...
    (OUT / "nexora_base.txt").write_text(args.base, encoding="utf-8")
    print(f"\nSaved LoRA adapter -> {OUT.relative_to(ROOT)}")
    print("Next:  python scripts/slm_infer.py")


if __name__ == "__main__":
    ap = argparse.ArgumentParser(description="LoRA fine-tune NEXORA SLM")
