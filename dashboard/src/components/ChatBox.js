...
export default function ChatBox({ provider }) {
  const [messages, setMessages] = useState([
    { role: 'bot', content: "I'm NEXORA. Ask me about fleet health, active escalations, or what to prioritize — I answer from the live state of your assets." },
  ]);
      ...
      <div className="chat-log" ref={logRef}>
        {messages.map((m, i) => (
          <div key={i} className={`chat-msg ${m.role === 'user' ? 'user' : 'bot'}`}>
            {m.role === 'bot' && <div className="who">NEXORA{m.meta ? ` · ${m.meta}` : ''}</div>}
            <div className="bubble">{m.content}</div>
          </div>
        ))}
        {busy && (
          <div className="chat-msg bot">
            <div className="who">NEXORA</div>
